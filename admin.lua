...
CMDs[#CMDs + 1] = {NAME = 'tweentpposition / ttppos [X Y Z]', DESC = 'Tween to coordinates (bypasses some anti cheats)'} glider = ;tppos 880.20 16.38 -602.79 ;ttppos 3027.75 7883.18 1060.45
...
addcmd('tpposition',{'tppos'},function(args, speaker)
	...
	-- split koordinat dengan koma
	local split = {}
	for val in string.gmatch(args[1], "([^,]+)") do
		table.insert(split, val)
	end

	if #split < 3 then return end
	local tpX, tpY, tpZ = tonumber(split[1]), tonumber(split[2]), tonumber(split[3])
	local char = speaker.Character
...
addcmd('tweentpposition',{'ttppos'},function(args, speaker)
	if #args < 1 then return end
	local coords = table.concat(args, " ")
	local split = {}
	for val in string.gmatch(coords, "([^,]+)") do
		table.insert(split, val)
	end
	if #split < 3 then return end
	local tpX,tpY,tpZ = tonumber(split[1]), tonumber(split[2]), tonumber(split[3])
	local char = speaker.Character
	if char and getRoot(char) then
		TweenService:Create(getRoot(char), TweenInfo.new(tweenSpeed, Enum.EasingStyle.Linear), {CFrame = CFrame.new(tpX, tpY, tpZ)}):Play()
	end
end)
